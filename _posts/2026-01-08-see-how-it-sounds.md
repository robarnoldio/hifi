...
Hw it works is you show up to any evaluation with some prior beliefs about how things work. And you go ahead and collect some observations in order to form some conclusions. And your brain had evolved to have a tendency to disregard the observations that don't confirm your prior beliefs, and give more significance to the observations that do.
...
Let's investigate the 90 Hz notch. The procedure here is using my handheld sound meter and kind of moving it around in space to see (and hear) how localized that notch is (spolier: super localized). To really make the demo vivid, the input signal is a sine wave at 90 Hz, which is free of information that is *not* going to trigger that effect. We're expecting a big change in the meter reading as we get to the listening position (where the yellow curve measurement above was taken).
...
The effect is quite vivid, around 15 dB lower, which tracks what we see on the graph nicely. It's quite a bit worse when only the Right channel is putting energy into the room. Having hte Left speaker also provise the test signal lessens this notch, but also reflects how I would normally use my system.
...
So we measured this, we confirmed our measurement with some further testing, and now we need to evaluate--is this a problem taht is worth correcting? Did I move my heavy couch for nothing? Are we winning?
